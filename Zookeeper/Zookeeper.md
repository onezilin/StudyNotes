...
当**集群重启**或**Leader 掉线**时，Zab 协议进入恢复模式，选举产生新的 Leader。一但出现崩溃，会导致数据不一致，Zab 的崩溃恢复开始起作用。有如下两个确保：
...
③ ZooKeeper 集群正式对外提供事务服务，并且 Leader 可以进行消息广播。如果有新的节点加入，该 Follower 节点自动进入恢复模式，同时进行步骤 ①② 一样的操作。
