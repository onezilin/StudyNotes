...
>
> **三级缓存的目的是为了延迟代理对象的创建**，正常情况下，Spring 会在依赖注入之后进行代理（AnnotationAwareAspectJAutoProxyCreator.postProcessAfterInitialization()）；发生循环依赖时，才会提前（实例化后、依赖注入前）生成代理实例(AnnotationAwareAspectJAutoProxyCreator.getEarlyBeanReference())。如果没有三级缓存，不管有没有发生循环依赖，A 每次都必须生成代理实例。
>
