...
![Solr_Home和Solr_Core目录](./Solr_Home和Solr_Core目录.png)
...
![boosting_query计算效果](./boosting_query计算效果.png)
...
- 分片路由机制：插入一条数据时，不用关心数据怎么进行分片、最后存入哪个分片。
...
![Elasticsearch写入操作执行流程](./Elasticsearch写入操作执行流程.png)
...
![Elasticsearch新增文档原理](Elasticsearch新增文档原理.png)
