...
广义上的零拷贝是指**CPU 不会将数据在内核空间和用户空间之间进行拷贝**；狭义上的零拷贝是指**CPU 完全不会参与数据拷贝的过程**，完全有 DMA 进行 I/O 操作。零拷贝技术实现的方式通常有 2 种：
...
理论上一个客户端连接多个服务器，但是从设计角度很鸡肋，所以一个客户端只能 Socket.connect()一次，连接一个服务器。
...
当此通道的 isWritable() 写缓冲区有空闲时为 true，一般来说写缓冲区总是有空闲的。
